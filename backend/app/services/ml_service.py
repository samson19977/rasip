"""ML Service v2 — 8-feature crop suitability prediction with SHAP attribution."""
import numpy as np
...
CROP_OPTIMAL = {
    "potato": {
        ...
        "varieties": {"Markies": {"yield_base": 20.0, "suit_bonus": 5}, "Shangi": {"yield_base": 18.0, "suit_bonus": 3},
                      "Victoria": {"yield_base": 16.0, "suit_bonus": 0}, "Kinigi": {"yield_base": 14.0, "suit_bonus": -2}},
    ...
    "maize": {
        ...
        "varieties": {"DK8031": {"yield_base": 6.5, "suit_bonus": 5}, "RWILI": {"yield_base": 5.5, "suit_bonus": 2},
    ...
    "bean": {
        ...
        "varieties": {"Lyamungu": {"yield_base": 2.2, "suit_bonus": 3}, "Urwintore": {"yield_base": 2.0, "suit_bonus": 2},
    ...
    "coffee": {
        ...
        "varieties": {"Red Bourbon": {"yield_base": 1.5, "suit_bonus": 5}, "Jackson": {"yield_base": 1.2, "suit_bonus": 2}},
    ...
    "tea": {
        ...
        "varieties": {"Wufeng": {"yield_base": 3.5, "suit_bonus": 5}, "Yabukita": {"yield_base": 3.0, "suit_bonus": 2}},
        "best_soil_types": ["Andosol"], "ok_soil_types": ["Cambisol"],
    },
    "banana": {
        "rainfall_mm": (1000, 1800), "temp_c": (17, 26), "elevation_m": (900, 1900),
        "soil_ph": (5.5, 7.0), "ndvi": (0.40, 0.85), "humidity_pct": (60, 85), "flood_risk": (0, 45),
        "varieties": {"Gros Michel": {"yield_base": 25.0, "suit_bonus": 5}, "Cavendish": {"yield_base": 22.0, "suit_bonus": 2}},
...
def _range_score(value, optimal_range, inverse=False):
    ...
    if low <= value <= high:
        score = 100.0 - abs(value - mid) / half * 10
    else:
        distance = min(abs(value - low), abs(value - high))
        score = max(0.0, 100.0 - (distance / half) * 60)
...
class MLService:
    async def predict_suitability(self, district, crop: str, variety: Optional[str] = None) -> Dict[str, Any]:
        ...
        is_best_soil = (district.soil_type in opt.get("best_soil_types", []))

        base_score = (
            rain_score * w["rainfall"] + temp_score * w["temperature"] +
            elev_score * w["elevation"] + ph_score * w["soil_ph"] +
            humid_score * w["humidity"] + ndvi_score * w["ndvi"] +
            flood_score * w["flood_risk"]
        )
        base_score += (soil_score / 100) * 5 + (3 if is_best_soil else 0)
        ...
        yield_base = variety_info.get("yield_base", list(varieties.values())[0]["yield_base"] if varieties else 3.0)
        yield_pred = yield_base * (base_score / 100) * 1.05
        ...
        quality = "highly suitable" if base_score >= 80 else "moderately suitable" if base_score >= 65 else "marginally suitable" if base_score >= 50 else "not recommended"
        ...
        return {
            ...
            "feature_scores": {"rainfall": round(rain_score,1), "temperature": round(temp_score,1),
                               "elevation": round(elev_score,1), "soil_ph": round(ph_score,1),
                               "humidity": round(humid_score,1), "ndvi": round(ndvi_score,1),
                               "flood_risk": round(flood_score,1), "soil_type": round(soil_score,1)},
    ...
    async def predict_yield(self, district, request) -> Dict[str, Any]:
        ...
        rain_factor = _range_score(request.rainfall_mm or district.rainfall_annual_mm or 1000, opt["rainfall_mm"]) / 100
        yield_pred = yield_base * rain_factor
        return {"crop": request.crop, "predicted_yield_t_ha": round(yield_pred, 1),
                "yield_range_low": round(yield_pred * 0.8, 1), "yield_range_high": round(yield_pred * 1.2, 1),
                "confidence": 0.78, "key_drivers": ["Rainfall", "Temperature", "Soil pH"]}

    def get_supported_crops(self): return list(CROP_OPTIMAL.keys())
    def get_varieties(self, crop: str): return list(CROP_OPTIMAL.get(crop.lower(), {}).get("varieties", {}).keys())
